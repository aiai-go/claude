...
/**
 * @aiai-go/claude — npm wrapper for the Python CLI
 *
 * Flow:
...
function findPython() {
  ...
  console.error(
    "\x1b[31m[@aiai-go/claude] 错误: 未找到 Python 3.10+\x1b[0m\n" +
...
function installBundledPackage(pythonCmd) {
  ...
  console.log("[@aiai-go/claude] 正在安装 Python 依赖...");
  ...
  for (const cmd of pipCmds) {
    ...
    if (result !== null) {
      console.log("[@aiai-go/claude] Python 依赖安装完成 ✓");
      return true;
    }
  }
  console.error(
    "\x1b[31m[@aiai-go/claude] 错误: 无法安装 Python 依赖\x1b[0m\n" +
...
function main() {
  ...
  child.on("error", (err) => {
    console.error(`[@aiai-go/claude] 启动失败: ${err.message}`);
    process.exit(1);
  });
}
