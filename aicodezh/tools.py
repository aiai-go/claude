...
def run_command(cmd: str, timeout: int = 30, allow_dangerous: bool = False) -> dict:
    ...
    if not allow_dangerous and _is_dangerous(cmd):
        return {
            "stdout": "",
            "stderr": f"⚠️ 命令被安全策略拦截（疑似危险操作）: {cmd}",
            "returncode": -1,
        }
    ...
    try:
        ...
        return {
            "stdout": "",
            "stderr": f"⚠️ 命令执行超时（{timeout}秒）: {cmd}",
            "returncode": -1,
        }
...
def run_python(code: str, timeout: int = 30) -> dict:
    ...
    try:
        ...
        return {
            "stdout": "",
            "stderr": f"⚠️ Python 代码执行超时（{timeout}秒）",
            "returncode": -1,
        }
    except Exception as e:
        return {
            "stdout": "",
            "stderr": f"⚠️ Python 代码执行失败: {e}",
            "returncode": -1,
        }
    finally:
        try:
...
TOOL_REGISTRY: dict[str, dict] = {
    "read_file": {
        "function": read_file,
        "description": "读取文件内容（自动检测编码）",
        "params": {"path": "文件路径"},
    },
    "write_file": {
        "function": write_file,
        "description": "写入文件（自动备份已有文件）",
        "params": {"path": "文件路径", "content": "文件内容"},
    },
    "list_files": {
        "function": list_files,
        "description": "列出目录中的文件（支持 glob 模式）",
        "params": {"path": "目录路径", "pattern": "匹配模式"},
    },
    "search_in_files": {
        "function": search_in_files,
        "description": "在文件中搜索内容（正则表达式）",
        "params": {"pattern": "搜索模式", "path": "目录路径", "glob": "文件匹配模式"},
    },
    "run_command": {
        "function": run_command,
        "description": "执行 Shell 命令",
        "params": {"cmd": "命令字符串", "timeout": "超时秒数"},
    },
    "run_python": {
        "function": run_python,
        "description": "执行 Python 代码片段",
        "params": {"code": "Python 代码", "timeout": "超时秒数"},
    },
    "analyze_project": {
        "function": analyze_project,
        "description": "分析项目结构（语言、框架、关键文件）",
        "params": {"path": "项目路径"},
    },
    "get_git_info": {
        "function": get_git_info,
        "description": "获取 Git 仓库信息（分支、状态、提交记录）",
        "params": {"path": "仓库路径"},
    },
}
