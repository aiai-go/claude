...
class CheckpointManager:
    ...
    def undo(self, checkpoint_id: Optional[str] = None) -> tuple[bool, str, list[str]]:
        ...
        if not checkpoints:
            return False, "没有可用的检查点", []

        # 找到目标检查点
        target: Optional[CheckpointInfo] = None
        if checkpoint_id is None:
            target = checkpoints[0]  # 最近的
        else:
        ...
        if target is None:
            return False, f"未找到检查点: {checkpoint_id}", []
        ...
        for file_path in target.files_backed_up:
            ...
            if not backup_path.exists():
                errors.append(f"备份文件丢失: {file_path}")
                continue
            ...
            try:
                ...
                reverted_files.append(f"[恢复] {file_path}")
            except (OSError, IOError) as e:
                errors.append(f"恢复失败 {file_path}: {e}")
        ...
        for file_path in target.files_created:
            if os.path.exists(file_path):
                try:
                    os.remove(file_path)
                    reverted_files.append(f"[删除] {file_path}")
                except (OSError, IOError) as e:
        ...
        if errors:
            ...
            msg = f"部分撤销完成 ({len(reverted_files)} 个文件), 但有错误: {error_msg}"
            return True, msg, reverted_files

        msg = f"已撤销到 {target.time_str} 的状态 ({len(reverted_files)} 个文件)"
        return True, msg, reverted_files
