...
DANGEROUS_PATTERNS: list[tuple[re.Pattern, str]] = [
    (re.compile(r"rm\s+-rf\s+/", re.IGNORECASE), "Recursively delete root directory"),
    (re.compile(r"rm\s+-rf\s+~", re.IGNORECASE), "Recursively delete home directory"),
    (re.compile(r"rm\s+-rf\s+\*", re.IGNORECASE), "Recursively delete all files"),
    (re.compile(r"mkfs\.", re.IGNORECASE), "Format disk"),
    (re.compile(r"dd\s+if=", re.IGNORECASE), "Direct disk write"),
    (re.compile(r">\s*/dev/sd", re.IGNORECASE), "Overwrite disk device"),
    ...
    (re.compile(r"chmod\s+777", re.IGNORECASE), "Set world-writable permissions"),
    (re.compile(r"curl.*\|\s*sh", re.IGNORECASE), "Pipe to shell from remote"),
    (re.compile(r"wget.*\|\s*sh", re.IGNORECASE), "Pipe to shell from remote"),
]
...
async def pre_bash_hook(
    ...
    for pattern, desc in DANGEROUS_PATTERNS:
        if pattern.search(command):
            warning = f"Dangerous command detected: {desc}"
            logger.warning(f"[Safety Hook] {warning} — cmd: {command[:80]}")
            _write_audit("Bash", f"BLOCKED: {command[:120]}", "blocked")

            return {
                "decision": "block",
                "reason": f"Command blocked by safety system. {warning}. Please use a safer alternative.",
                "systemMessage": f"[Safety Warning] {warning}\nCommand: {command[:80]}",
                "hookSpecificOutput": {
...
async def pre_write_hook(
    ...
    for cp in critical_paths:
        if file_path.startswith(cp):
            warning = f"Attempting to write to critical system path: {file_path}"
            logger.warning(f"[Safety Hook] {warning}")
            _write_audit("Write/Edit", f"BLOCKED: {file_path}", "blocked")
            return {
                "decision": "block",
                "reason": f"Write to critical system path blocked: {file_path}. Please confirm this is intended.",
                "systemMessage": f"[Safety Warning] {warning}",
                "hookSpecificOutput": {
                    "hookEventName": "PreToolUse",
                    "permissionDecision": "deny",
